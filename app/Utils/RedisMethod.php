...
class RedisMethod
{
    ...
    /**
     * 获取redis中所有的key值
     * @param int $count 一次取出多少个key值(默认一次1000)
     * @param string $pattern key值匹配模式(默认匹配所有内容)
     * @param int $iterator 迭代器，初始化为空
     * @return array
     */
    public function getAllKeys($count = 1000, $pattern = '*', $iterator = 0)
    {
        $arrKeys = [];
        while($keys = $this->redis->scan($iterator, $pattern, $count)) {
            $arrKeys = array_merge($keys, $arrKeys);
        }
